...
All functions accept either Φ directly or use the convenience *_from_xy variants that internally build 
Φ=[1,x]  for 1D inputs.

Shapes: y can be (N,) or (N,1); it will be treated as a column vector internally.

optimize_hyperparameters_xy uses SciPy’s minimize with gradients from JAX; it returns the optimized precisions 
(α,β).
"""